...
const { isAuthenticated } = require("../middleware/authing.js");

expenseRouter.post("/add", isAuthenticated, async (req, res) => {
  try {
...
expenseRouter.get("/list", isAuthenticated, async (req, res) => {
  try {
    const userId = req.session.user.id;
    const {
      note,
...
expenseRouter.put("/edit/:id", isAuthenticated, async (req, res) => {
  try {
...
expenseRouter.delete("/delete/:id", isAuthenticated, async (req, res) => {
  try {
    const id = req.params.id;
    const userId = req.session.user.id;
    const selectedExpense = await Expense.findByPk(id);
    ...
    if (numDeleted === 0) {
      ...
      const userBalance = await UserBalance.findByPk(selectedExpense.balanceId);
