...
incomeRouter.post("/add", isAuthenticated, async (req, res) => {
  try {
...
incomeRouter.get("/list", isAuthenticated, async (req, res) => {
  try {
    const userId = req.session.user.id;
    const {
      note,
...
incomeRouter.put("/edit/:id", isAuthenticated, async (req, res) => {
  try {
...
incomeRouter.delete("/delete/:id", isAuthenticated, async (req, res) => {
  try {
    const id = req.params.id;
    const userId = req.session.user.id;
    const selectedIncome = await Income.findByPk(id);
    ...
    if (numDeleted === 0) {
      ...
      const userBalance = await UserBalance.findByPk(selectedIncome.balanceId);
