...
goalRouter.get("/list", isAuthenticated, async (req, res) => {
  try {
    const userId = req.session.user.id;
...
goalRouter.put("/edit/:id", isAuthenticated, async (req, res) => {
  try {
...
goalRouter.delete("/delete/:id", isAuthenticated, async (req, res) => {
  try {
    const id = req.params.id;
    const userId = req.session.user.id;
...
goalRouter.delete("/complete/:id", isAuthenticated, async (req, res) => {
  try {
    const id = req.params.id;
    const userId = req.session.user.id;
    ...
    if (selectedGoal.status) {
      ...
      return res.status(200).json({ message: "Your Goal has been completed" });
    }
  } catch (error) {
